...
echo 'alias tnm="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/TNMEcosystem/core" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/TNMEcosystem/core'"
...
rm -rf "$HOME/.config/@tnm"
rm -rf "$HOME/.config/@tnmchain"
rm -rf "$HOME/.config/tnm-core"
